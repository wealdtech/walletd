...
func (s *Service) SignBeaconAttestation(ctx context.Context, req *pb.SignBeaconAttestationRequest) (*pb.SignResponse, error) {
	res := &pb.SignResponse{}
	...
	rules := s.ruler.Rules("sign beacon attestation", accountName)
	result := backend.RunRules(rules,
		...
		func() (*backend.State, error) {
			return s.storage.FetchBeaconAttestationState(account.PublicKey().Marshal())
		},
		func(table *lua.LTable, state *backend.State) error {
			table.ForEach(func(k, v lua.LValue) {
				state.Store(k.String(), v)
			})
			return s.storage.StoreBeaconAttestationState(account.PublicKey().Marshal(), state)
		})
	switch result {
